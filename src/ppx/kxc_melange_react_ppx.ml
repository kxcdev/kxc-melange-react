...
==== kxc-melange-react-ppx (https://github.com/kxcdev/kxc-melange-react-ppx) ====
...
along with this program.  If not, see <https://www.gnu.org/licenses/>.
                                                                              *)
(* Acknowledgements --- Melange (https://melange.re) --- The current/initial
version or a significant portion of this file is based on code originated
from the Melange project (https://melange.re https://github.com/melange-re)
and is licensed under the GNU General Public License:

Copyright (C) 2015-2016 Bloomberg Finance L.P.
Copyright (C) 2017- Hongbo Zhang 
...
along with this program.  If not, see <https://www.gnu.org/licenses/>.
                                                                              *)
(* code originally adopted from
https://github.com/melange-re/melange/blob/1.0.0/reactjs-jsx-ppx/reactjs_jsx_ppx_v3.ml
                                                                              *)
open Ppxlib
open Ast_helper
open Asttypes
...
let jsxMapper =
  ...
  let transformComponentDefinition ~ctxt mapper structure returnStructures =
    let (|&!>) l f = List.mapi f l in
    let (|&>) l f = List.map f l in
    let make_helper ~loc fnName propsHelperName helperName namedArgListWithKeyAndRef =
      let exp ?(loc = loc) s = Exp.ident ~loc {txt = Lident s; loc} in
      let mexp ?(loc = loc) (m, s) =
        Exp.ident ~loc {txt =  Ldot (Lident m, s); loc} in
      let exp_unit = Exp.construct ~loc {txt=Lident "()"; loc} None in
      let pat ?(loc = loc) s = Pat.var ~loc {txt = s; loc} in
      let pat_unit ?(loc = loc) () = Pat.construct ~loc {txt=Lident "()"; loc} None in
      let pat_any = Pat.any ~loc () in
      let wrap ?(loc = loc) (m, f) e =
        Exp.apply ~loc (Exp.ident ~loc { loc; txt = Ldot (Lident m, f) })
          [Nolabel, e] in
      let name f l d = match l with
        | Nolabel -> d
        | (Labelled n | Optional n) -> f n in
      let children_style, arg_list =
        match namedArgListWithKeyAndRef
              |> List.partition
                   (fun (l, _, _, _) ->
                     name ((=) "children") l false) with
        | [], l -> `no_child, l
        | [Optional _, _, loc, typ], l -> `optional_children (loc, typ), l
        | [Labelled _, _, loc, typ], l -> `always_children (loc, typ), l
        | _ -> invalid_arg "multiple children labelled argument"
      in
      let body =
        let app_arg_list =
          arg_list |&!>
            (fun i -> function
              | Nolabel, _, loc, _ -> Nolabel, (exp ~loc ("_"^string_of_int i))
              | Labelled l as lbl, _, loc, _ -> lbl, (exp ~loc l)
              | Optional l as lbl, _, loc, _ -> lbl, (exp ~loc l))
        in
        let children_app_arg =
          let tr ?(loc = loc) x =
            x |> wrap ~loc ("Array", "of_list")
          in
          match children_style with
          | `no_child -> []
          | `optional_children (loc, _) ->
             [Nolabel, tr ~loc (exp ~loc "children")]
          | `always_children (loc, _) ->
             [Nolabel, tr ~loc (exp ~loc "children")]
        in
        let creationFnName = match children_style with
          | `no_child -> "React", "createElement"
          | (`optional_children _ | `always_children _) ->
             "React", "createElementVariadic"
        in
        (Exp.apply ~loc
           (Exp.apply ~loc
              (mexp ~loc creationFnName)
              [Nolabel, exp (fnName)])
           ([(Nolabel,
              (Exp.apply ~loc (exp (propsHelperName))
                 ((Nolabel, exp_unit) ::
                    (app_arg_list)
                 )
            ))] @ children_app_arg)) in
      let children_fun_arg =
        match children_style with
        | `no_child -> pat_unit ~loc ()
        | (`optional_children (loc, _) | `always_children (loc, _)) ->
           pat ~loc "children"
      in
      Vb.mk ~loc
        (pat (helperName))
        (List.fold_right (fun (l, _, _, _) acc ->
             (Exp.fun_ l None (name pat l pat_any) acc))
           arg_list
           (Exp.fun_ Nolabel None children_fun_arg body))
      (* |> (fun x -> [x]) *)
      (* |> Str.value ~loc Nonrecursive *)
      (* (Format.printf "%a@." Astlib.Pprintast.structure_item helper); *)
    in
    ...
    | {
        ...
        pstr_desc =
          Pstr_primitive
            ({ pval_name = ({ txt = fnName } as pval_name); pval_attributes; pval_type } as
            value_description);
      } as pstr -> (
        ...
        | [ _ ] ->
            ...
            let helper =
              let loc = pstr_loc in
              Str.value ~loc Nonrecursive
                [make_helper ~loc (fnName^"'") (fnName^"Props") fnName namedArgListWithKeyAndRef] in
                  ...
                  Pstr_primitive
                    {
                      ...
                      pval_name = { pval_name with txt = fnName^"'"};
            ...
            in
            externalPropsDecl :: newStructure :: helper :: returnStructures
        | _ ->
            raise
              (Invalid_argument
    ...
    | { pstr_loc; pstr_desc = Pstr_value (recFlag, valueBindings) } ->
        ...
        let mapBinding binding =
          if hasAttrOnBinding binding then
            ...
            in
            let modifiedBinding binding newFnName =
              ...
              let wrapExpressionWithBinding expressionFn expression =
                Vb.mk ~loc:bindingLoc
                  ...
                  (Pat.var ~loc:bindingPatLoc
                     { loc = bindingPatLoc; txt = newFnName })
            ...
            in
            let newFnName, helperName = match fnName with
              | "make" -> "make", "elem"
              | n -> n^"'", n in
            let bindingWrapper, hasUnit, expression = modifiedBinding binding newFnName in
            ...
            in
            let bindings, newBinding, extraBindings =
              let helperNamedArgList =
                namedArgListWithKeyAndRef
                |&> fun (l, c, _, _, loc, t) -> l, c, loc, t in
              match recFlag with
              | Recursive ->
                  ( [
                      bindingWrapper
                    ...
                    None,
                    [
                      make_helper ~loc:binding.pvb_loc
                        newFnName (fnName^"Props") helperName
                        helperNamedArgList;
                  ]
                  )
              | Nonrecursive ->
            ...
            in
            (Some externalDecl, bindings, newBinding, extraBindings)
          else (None, [ binding ], None, [])
          [@@raises Invalid_argument]
        in
        let structuresAndBinding = List.map mapBinding valueBindings in
        let otherStructures (extern, binding, newBinding, extraBindings)
            (externs, bindings, newBindings, extraBindingsAcc) =
          let externs =
            match extern with
          ...
          let newBindings =
            ...
            | None -> newBindings
          in
          (externs, binding @ bindings, newBindings, extraBindings @ extraBindingsAcc)
        in
        let externs, bindings, newBindings, extraBindings =
...
let () =
  Driver.V2.register_transformation "kxc-melange-react-ppx"
